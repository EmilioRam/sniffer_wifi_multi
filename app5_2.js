...
const snifferId = 'sniffer5Ghz_2(ch104-140)'; //cambiar por id del sniffer en el que se ejecute
...
var fecha = new Date().toLocaleString().replaceAll("/", "-");

var csvWriter = createCsvWriter({
    path: `./docs/${snifferId}_${fecha}.csv`,
    fieldDelimiter: ";",
    header: [
...
//cambiar a cada x horas, (nuevo archivo)
cron.schedule('0 0 * * *', () => {
    console.log('cambiando archivo csv'.yellow);
    fecha = new Date().toLocaleString().replaceAll("/", "-");
...
console.log("esperando 30 seg a que se inicie script e rotación de canales...");
...
function init() {
    ...
    var pcapSession = pcap.createSession('wlan1', { filter: 'type mgt subtype probe-req' });
